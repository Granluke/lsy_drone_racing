...
class PathPlanning:
    ...
    def generate_gate_waypoints(self, i, gate):
        ...
        if i < len(self.gates) - 1:
            self.check_and_avoid_gate_intersect(i, gate, z_center, after_wp)
        else:
            self.go_around_points.append(([]))

    

    def check_and_avoid_gate_intersect(self, i, gate, z_center, after_wp):
    ...
    def point_in_gate(self, point, gate_center, yaw):
        gate_width = 0.45 + self.buffer
        gate_height = 0.45 + self.buffer
