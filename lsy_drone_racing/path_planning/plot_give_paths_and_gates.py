...
import ast
import json
from lsy_drone_racing.path_planning.calc_path_through_gates import create_gate, create_cylinder, OBSTACLE_DIMENSIONS
...
def plot_gates_and_obstacles(csv_file):
    ...
    for cylinder in obstacles:
        ...
        x_grid, y_grid, z_grid = create_cylinder(x, y, z, OBSTACLE_DIMENSIONS['height'], OBSTACLE_DIMENSIONS['radius'])
    ...
    ax.plot(data_actual['Actual position X'], data_actual['Actual position Y'], data_actual['Actual position Z'], '-', label='Actual Position', color='black')
