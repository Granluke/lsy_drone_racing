...
class PathPlanning:
    ...
    def generate_gate_waypoints(self, i, gate):
        ...
        if len(self.waypoints) >= 1:
            ...
            if dist_last_wp_before_wp > 1.4:
                ...
                self.waypoints.append([x + factor_for_2nd_before_point * np.sin(yaw), 
    ...
    def check_and_avoid_gate_intersect(self, i, gate, z_center, after_wp, factor_for_2nd_before_point):
        ...
        intersects, intersection_point = self.line_intersects_plane(line_start, line_end, plane_point, plane_normal)
        if intersects:
        ...
        if intersects and self.point_in_gate(intersection_point, np.array([x, y, z_center]), yaw):
            ...
            self.waypoints[-1] = [x + factor_for_2nd_avoidancy_point * (self.buffer + 0.1) * np.sin(yaw), 
                                y - factor_for_2nd_avoidancy_point * (self.buffer + 0.1) * np.cos(yaw),
                                    z_center - 0.05]
            avoidance_wp2 = [x + factor_for_2nd_avoidancy_point * (self.buffer + 0.1) * np.sin(yaw), 
                                y - factor_for_2nd_avoidancy_point * (self.buffer + 0.1) * np.cos(yaw), 
                                z_center + 0.3]  
    ...
    def plot_gates_and_obstacles(self, path_before_obstacle_avoidance, path_after_obstacle_avoidance):
        fig = plt.figure()
        ...
        for intersection_point in self.intersection_points:
            if intersection_point is not None:
                ...
                ax.scatter([round(p[0], 2)], [round(p[1], 2)], [round(p[2], 2)], color='orange', s=50)
    ...
    # ax.scatter(*[-0.12161042,0.56976572,0.525], color='orange', s=60)
        ...
        # Plot the path
        ax.plot(path_before_obstacle_avoidance[0], path_before_obstacle_avoidance[1], path_before_obstacle_avoidance[2], 'g', label='Original Path')
        ax.plot(path_after_obstacle_avoidance[0], path_after_obstacle_avoidance[1], path_after_obstacle_avoidance[2], 'r', label='Adjusted Path')
        ...
        patch= Line2D([0], [0], label='Waypoints', color='black', marker='o', markersize=6, 
            linestyle='')
        patch2= Line2D([0], [0], label='Original Before Gate Waypoints', color='blue', marker='o', markersize=6, 
            linestyle='')
        patch3= Line2D([0], [0], label='Original After Gate Waypoints', color='red', marker='o', markersize=6, 
            linestyle='')
        patch4= Line2D([0], [0], label='Gate Intersections', color='orange', marker='o', markersize=6, 
            linestyle='')
